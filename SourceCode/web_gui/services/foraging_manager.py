"""Foraging state management — tracks active research jobs and pause/yield state."""
...
class ForagingManager:
    """Thread-safe foraging state manager.

    Tracks active foraging jobs, paused state, and yield windows.
    """
    ...
    def rows_for_profile(
        ...
        for item in active_jobs.values():
            ...
            rows.append({
                ...
                "raw_path": str(job_row.get("raw_path", "")).strip(),
    ...
    def request_yield(self, seconds: float = 90.0) -> None:
        now_ts = time.time()
        target = now_ts + max(5.0, float(seconds))
        with self._lock:
            prev = float(self._state.get("yield_until", 0.0) or 0.0)
            self._state["yield_until"] = max(prev, target)
            self._state["updated_at"] = _now_iso()

    def should_yield(self) -> bool:
        now_ts = time.time()
        with self._lock:
            until = float(self._state.get("yield_until", 0.0) or 0.0)
            if now_ts >= until:
                if until != 0.0:
                    self._state["yield_until"] = 0.0
                    self._state["updated_at"] = _now_iso()
                return False
            return True
